...
sed -i "/uci commit network/i\uci set network.lan.gateway='192.168.50.250'" $ZZZ               # IPv4 网关
sed -i "/uci commit network/i\uci set network.lan.broadcast='192.168.50.255'" $ZZZ           # IPv4 广播
sed -i "/uci commit network/i\uci set network.lan.dns='202.99.224.68 223.5.5.5 114.114.114.114'" $ZZZ     # DNS(多个DNS要用空格分开)
...
sed -i "/uci commit system/i\uci set system.@system[0].hostname='OpenWrt-HAOZI'" $ZZZ         # 修改主机名称为OpenWrt-123

sed -i "s/OpenWrt /HAOZI Compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ    # 增加自己个性名称281677160
